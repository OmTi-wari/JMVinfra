  ...
  return (
    ...
    >
      <div className="grid gap-10 lg:grid-cols-[0.72fr_1.28fr]">
        <aside className="h-fit rounded-2xl bg-jmv-brown p-7 text-white lg:sticky lg:top-28">
          <p className="eyebrow text-jmv-orange">Project profile</p>
          <dl className="mt-6 space-y-5">
            {meta.map(([label, value]) => (
              <div key={label} className="border-b border-white/10 pb-4">
                <dt className="text-xs uppercase tracking-[0.12em] text-white/50">{label}</dt>
        ...
        <div>
          <section>
            <p className="eyebrow text-jmv-orange">Executive summary</p>
            <h2 className="mt-3 text-3xl font-bold text-jmv-brown">
              Built for the demands of a live infrastructure environment.
            </h2>
            <p className="mt-5 text-base leading-8 text-jmv-charcoal/70">{project.summary}</p>
          </section>

          <section className="mt-14">
            <p className="eyebrow text-jmv-orange">Scope of execution</p>
            <ul className="mt-5 grid gap-3 sm:grid-cols-2">
              {project.scopeOfExecution.map((item) => (
                <li
                  key={item}
          ...
          <section className="mt-14">
            <p className="eyebrow text-jmv-orange">Engineering challenges overcome</p>
          ...
          <section className="mt-14">
            <p className="eyebrow text-jmv-orange">Project gallery</p>
            <div className="mt-5">
              <ImageLightbox
      ...
      <section className="mt-16 border-t border-border pt-10">
        <div className="flex items-center justify-between">
          <div>
            <p className="eyebrow text-jmv-orange">Related projects</p>
            <h2 className="mt-3 text-3xl font-bold text-jmv-brown">More work in this portfolio.</h2>
          </div>
          <Link
            to="/projects"
        ...
        <div className="mt-7 flex snap-x gap-5 overflow-x-auto pb-4">
          {related.length ? (
            related.map((item) => (
              ...
              >
                <p className="eyebrow text-jmv-orange">{item.status}</p>
                <h3 className="mt-3 font-bold text-jmv-brown">{item.title}</h3>
                <p className="mt-2 text-sm text-jmv-charcoal/65">{item.location}</p>
              </Link>
            ))
          ) : (
            <p className="text-sm text-jmv-charcoal/65">
              [Related Project Profile Pending Verification]
            </p>
          )}
        </div>
      </section>
