...
const offices = [
  {
    ...
    map: "[Interactive Mumbai Map Placeholder]",
  },
  {
    ...
    map: "[Interactive Ahmedabad Map Placeholder]",
  },
];
  ...
  return (
    ...
    >
      <div className="grid gap-6 lg:grid-cols-2">
        {offices.map((office) => (
          <article key={office.city} className="overflow-hidden rounded-[1.75rem] border border-border bg-white shadow-soft">
            <div className="flex min-h-56 items-center justify-center bg-jmv-brown p-8 text-center">
              <div>
                <MapPin className="mx-auto h-8 w-8 text-jmv-orange" />
                <p className="mt-4 text-2xl font-bold text-white">{office.map}</p>
              </div>
            </div>
            <div className="p-7">
              ...
              <div className="mt-7 flex flex-wrap gap-3">
                <a href={phoneHref} className="inline-flex items-center gap-2 rounded-full bg-jmv-orange px-5 py-3 text-sm font-semibold text-white hover:bg-jmv-orange-burnt">
                  <Phone className="h-4 w-4" /> Call office
                </a>
                <a href={`${whatsappHref}?text=Hello%20JMV%20${encodeURIComponent(office.city)}`} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 rounded-full border border-border bg-jmv-grey px-5 py-3 text-sm font-semibold text-jmv-brown hover:border-jmv-orange">
      ...
      <section className="mt-12 rounded-[2rem] bg-jmv-orange p-8 text-white md:p-10">
        ...
        <div className="mt-6 flex flex-wrap gap-3">
          <a href={phoneHref} className="inline-flex items-center gap-2 rounded-full bg-white px-5 py-3 text-sm font-semibold text-jmv-orange">
            <Phone className="h-4 w-4" /> {contact.phone}
          </a>
          <a href={`mailto:${contact.email}`} className="inline-flex items-center rounded-full border border-white/40 px-5 py-3 text-sm font-semibold text-white">
            {contact.email}
          </a>
        </div>
      </section>
